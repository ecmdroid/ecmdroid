...
public class CellEditorDialogFragment extends DialogFragment implements android.view.View.OnClickListener, OnKeyListener {
	...
	public interface CellEditorDialogListener {
		void onCellValueChanged(int pos, byte value);
	}
	 ...
	 */
	public static CellEditorDialogFragment newInstance(CellEditorDialogListener listener, int offset, byte value) {
	...
	public boolean onKey(View v, int keyCode, KeyEvent event) {
		...
		try {
			...
			int value = Integer.valueOf(input.length() == 0 ? "0" : input);
			getArguments().putByte(VALUE, (byte) value);
