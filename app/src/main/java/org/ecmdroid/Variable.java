...
public class Variable implements Cloneable {
	...
	public String toString() {
		String sb = "Variable[id: " + id + ", name:" + name +
				", ECM: " + etype + ", type: " + type +
				", size: " + size + ", offset: " + offset +
				", unit: " + unit + ", scale:" + scale +
				", trn: " + translate + ", high: " + high +
				"]";
		return sb;
	}
		...
		if (tmp != null && co >= 0 && co + size <= tmp.length) {
			for (int s = 0; s < (size / width); s++) {
				...
				if (type == DataType.BITS || type == DataType.BITFIELD) {
					rawValues[s] = (short) (value & 0xffff);
					...
					rawValues[s] = v;
					if ("0".equals(format)) {
						rawValues[s] = ((Double) rawValues[s]).intValue();
					}
				} else if (type == DataType.STRING) {
	...
	public void parseValueAt(int index, Object value) throws NumberFormatException {
		if (value != null) {
			...
			if ("0".equals(format)) {
				rawValues[index] = ((Double) rawValues[index]).intValue();
			}
			formatValueAt(index);
		}
	}
